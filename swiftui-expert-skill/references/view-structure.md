...
Use for small, simple sections (a few views, no expensive computation) that don't affect performance. Extract to a separate `struct` when the section is complex or needs to be skipped during re-evaluation.
